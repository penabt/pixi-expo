...
import { Application, Graphics, Text as PixiText, Container, Ticker } from 'pixi.js';
...
export default function App() {
  ...
  const handleApplicationCreate = useCallback((app: Application) => {
    ...
    // Create a red square
    const square = new Graphics().rect(-50, -50, 100, 100).fill({ color: 0xff0000 });
    shapes.addChild(square);

    // Create a blue circle
    const circle = new Graphics().circle(0, 100, 40).fill({ color: 0x0000ff });
    shapes.addChild(circle);

    // Create a green triangle
    const triangle = new Graphics().poly([0, -80, 40, 0, -40, 0]).fill({ color: 0x00ff00 });
    triangle.y = -100;
